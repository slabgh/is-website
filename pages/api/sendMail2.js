...
export default async function handler(req, res) {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      subject: `Ticket from ${name}`, // Subject line
      html: `
        <h3>${name} has raised a ticket</h3>
    ...
    // Success response
    res.status(200).json({ message: 'Your Ticket has been sent!' });
  } catch (error) {
    console.error('Email sending failed:', error);
    res.status(500).json({ message: 'Failed to send your ticket request.' });
  }
}
